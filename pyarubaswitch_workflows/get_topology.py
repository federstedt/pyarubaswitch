...
from pyarubaswitch.mac_table import MacTableElement
    ...
    def get_devices_csv(self, csv_filename):
        ...
        with open(csv_filename, mode='r', encoding='utf-8-sig') as csv_file:
            ...
            for line in csv_reader:
                mac_obj = MacTableElement(
                    mac_address=line['mac_address'],
                    port_id=line['port'],
                    vlan_id=line['vlan_id'],
                    switch_ip=line['switchip'],
                )
        ...
        with open(client_file, 'w', encoding='utf-8-sig') as f:
            ...
            for switch_obj in topology_list:
                for client in switch_obj.clients:
                    row = [
                        switch_obj.switch_ip,
                        client.mac_address.replace('-', ''),
                        client.port_id,
                        client.vlan_id,
                        '',
                    ]
        ...
        for switch in self.switches:
            ...
            if not switch_client.api_client.error:
                ...
                for entry in mac_table:
                    ...
                    if (
                        entry.port_id not in uplink_ports
                        and entry.port_id not in wireless_ports
                        and entry.vlan_id not in self.exlude_vlans
                        and 'Trk' not in entry.port_id
                    ):
                        num_clients += 1
                        clients.append(entry)
                    elif (
                        entry.port_id in wireless_ports
                        and entry.vlan_id not in self.exlude_vlans
                    ):
                ...
                max_clients = 2
                multi_ports = self.get_multi_client_port(
                    max_clients, clients
                )  # more than 1 client = multiclientport

                print(
                    f'Non AP Ports with more than {max_clients} clients, suspected unmanaged switch on ports (aka fulswitch):'
                )
                print(multi_ports)

                print(
                    f'Num ignored clients (found on uplink / ignored vlan): {len(ignored_entrys)}'
                )

                print(f'number of mac-entrys in table: {num_entrys}')
                print(
                    f'number of wired clients on switch (as in exlude found on uplinkports): {num_clients}'
                )
                # create switchobject
                sw_obj = SwitchInfo(
                    switch_ip=switch,
                    ...
                    lldp_devices=lldp_data,
                    number_mac_entrys=num_clients,
                    unmanaged_neighbor_ports=multi_ports,
                )
